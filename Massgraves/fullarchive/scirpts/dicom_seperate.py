...
OUTPUT_PATH = r"/Users/yoad/Desktop/CLS year 2/SVandVR/Massgraves/data/nii_exports"
...
def save_volume_as_nifti(vtk_image, direction_matrix, output_filepath, series_metadata):
    """
    Save volume as NIfTI file using SimpleITK
    
    Args:
        vtk_image: vtkImageData object
        direction_matrix: 3x3 orientation matrix
        output_filepath: Path to save .nii file
        series_metadata: Metadata dict with series info
    """
    print(f"\n  Saving as NIfTI file...")
    
    # Get dimensions and data from VTK image
    ...
    # Reshape to 3D (VTK stores as 1D, needs to be reshaped to Z,Y,X for SimpleITK)
    volume_3d = numpy_array.reshape(dims[2], dims[1], dims[0])  # Z, Y, X
    ...
    # Set direction matrix (flattened row-major order)
    sitk_image.SetDirection(direction_matrix.flatten().tolist())
    ...
    print(f"  ✓ Saved: {os.path.basename(output_filepath)} ({file_size_mb:.2f} MB)")
    print(f"    Dimensions: {dims[0]}×{dims[1]}×{dims[2]}")
    print(f"    Spacing: [{spacing[0]:.3f}, {spacing[1]:.3f}, {spacing[2]:.3f}] mm")
    print(f"    Full path: {output_filepath}")
...
def main():
    ...
    # Load each series and save as NIfTI
    saved_count = 0
    for series_info in primary_series:
        ...
        # Create safe filename from series description
        desc = series_info['metadata']['SeriesDescription']
        safe_desc = "".join(c if c.isalnum() or c in (' ', '-', '_') else '_' for c in desc)
        safe_desc = safe_desc.replace(' ', '_')
        
        # Create filename: DATASET_SeriesNumber_Description.nii
        filename = f"{DATASET}_Series{series_info['number']:02d}_{safe_desc}.nii"
        output_filepath = os.path.join(output_dir, filename)
        
        # Save as NIfTI
        save_volume_as_nifti(vtk_volume, direction, output_filepath, series_info['metadata'])
        saved_count += 1
        
        print(f"\n  Progress: {saved_count}/{len(primary_series)} series saved")
    
    print(f"\n{'='*100}")
    print(f"✓ ALL SERIES SAVED TO NIFTI")
