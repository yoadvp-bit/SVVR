#!/usr/bin/env python3
"""
BODY STITCHER WITH SMALL ROTATION OPTIMIZATION
===============================================
Copy of bodystitcher.py with:
- NO 180° rotation check (removed)
- ADDED small rotation check (±20°) during Z overlap search

Otherwise IDENTICAL to bodystitcher.py
"""
...
OUTPUT_DIR = os.path.join(FINALMG_DIR, 'nifti-stitched-smallrot')
...
# ============================================================================
# IMPROVED: MULTI-SCALE Z SEARCH WITH SMALL ROTATION CHECK
# ============================================================================
...
def multi_scale_z_search_with_rotation(torso_img, legs_img, verbose=True):
    """
    Multi-scale search for optimal Z overlap with small rotation optimization.
    
    NEW: Tests small rotations (±20°) for each Z overlap position.
    ...
    4. Return global best overlap AND best rotation angle
    """
    print("    Multi-scale Z overlap search WITH small rotation check...")
    ...
    # ========== STAGE 1: COARSE SEARCH (no rotation) ==========
...
def stitch_body(name, volume_paths):
    ...
    MODIFIED: No 180° rotation check, but WITH small rotation optimization.
    """
    print("=" * 80)
    ...
    # Multi-scale Z search WITH small rotation
    print("\n  Z Overlap Search WITH Small Rotation:")
...
def visualize_result(img, name):
    ...
    window.SetWindowName(f"Stitch with Small Rotation: {name}")
...
def main(visualize=True):
    ...
    """
    print("=" * 80)
    print("BODY STITCHER WITH SMALL ROTATION OPTIMIZATION")
    print("NO 180° rotation check + Small rotation (±20°) during Z search")
