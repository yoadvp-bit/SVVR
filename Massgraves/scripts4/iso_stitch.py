...
def stitch_two_volumes(vol1, vol2):
    ...
    # Place vol2 data ONLY where vol1 is empty (air)
    # This prevents double visualization in overlap regions
    mask2 = resampled2_array > -900
    empty_in_vol1 = union_array <= -900
    mask2_non_overlap = mask2 & empty_in_vol1
    
    union_array[mask2_non_overlap] = resampled2_array[mask2_non_overlap]
    
    overlap_voxels = np.sum(mask2 & ~empty_in_vol1)
    if overlap_voxels > 0:
        print(f"        Removed {overlap_voxels} overlapping voxels from new volume")
